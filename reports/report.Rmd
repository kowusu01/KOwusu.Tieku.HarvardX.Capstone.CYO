...
__Occupation__. 
Another import variable is _occupation_, a categorical variable with 15 levels,  one missing (?) category.  
...
Education wise, the females are equally educated as the men, but the average age for females is a bit lower than that of the males, meaning there are more younger women in the workforce (probably earning lower salaries), but am not sure if that explains the difference. 
...
#### Performance metric.  
In this projet, I use _AUC_ (Area Under the Curve) as the metric for selecting best models during cross validation. In the confusion matrix,  use Kappa instead of accuracy when selecting best model. Kappa is often preferred to accuracy when dealing with imbalanced dataset which is the case in this project. In th caret training function, the _metric_ will be set to __ROC__, to indicate to caret to select best model using _AUC_ rather than Accuracy.  

\newpage

### KNN Models
First, I trained two KNN models and picked the best performinng one. My approach was, first, I trained a model using caret defaults (knn_model_1). In knn, there is only one parameter to tune k. In the dafault  model, I used a tuneLenght of 10 to allow caret to select 10 values for k. This model provided a _best_ value for k from the cross validation.  I fit that model to the entire training set, use the fit to make predictions on the test.

I take the _best k_ from the first model, and expanded around it to provide more tuning values for k and used it to train another model (knn_model_2). I fitted the new model on the entire training set, and made another predictions on test. I then compare the two models. 
...
With this model, the same sample (5K observations from the training set) with the same cross validation were used in an attempt to tune mtry. From model 1, i the best mtry was ```r rf_model_1_results$mtry[which.max(rf_model_1_results$ROC)] ``` . The idea was to expand the around the mtry obtained from model 1 and  see if there is a better mtry. With this approach, the best cross validation ROC for model 2 is achieved with mtry = ```r rf_model_2_results$mtry[which.max(rf_model_2_results$ROC)]```.  
...
In model 3, mtry from model 2 is used and held it constant to find the best nodesize.  Different models were fitted across different values of nodesize and the one with best Kappa was selected. The plot below shows the values of Kappa across different values for nodesize in model 3.
...
From table above, randomForest model 3 ( _rf_model_ ) is the best performing model. Even though the performance between kNN and the last two models from randomForest are very close in terms of accuracy, it it clear that randomForest model 3 is a better model using Kappa as a metric. Therefore, I still selected randomForest model 3 as the final model to apply on the validation because. The selected model used mtry of __15__ and nodesize of __25__.
...
The final confusion matrix shows an Accuracy of __86%__, Kappa of __0.58__, and  Sensitivity of __0.6__. 
