...
```{r 01_source_include, echo=FALSE, message=FALSE, include=FALSE}
  ...
  adult_income_train <- readRDS( file.path( rda_path, "adult_income_train.rds"))
  
  ## the original validation set
  adult_income_validation  <- readRDS(file.path( rda_path, "adult_income_validation.rds"))
  
  # this is the engineered for visualization
  adult_income <- readRDS(file.path( rda_path, "adult_income.rds"))
  
  # read the final dataset used for training after splitting
  training_set_final <- readRDS(file.path( rda_path, "training_set_final.rds"))
  
  # the test (holdout) set
  test_set_final <- readRDS(file.path( rda_path, "test_set_final.rds"))
  ...
  knn_model_1_results <- readRDS(file.path( rda_path, "knn_model_1_cv_results.rds"))
  knn_model_2_results <- readRDS(file.path( rda_path, "knn_model_2_cv_results.rds"))
  
  knn_confusionMatrix_1 <- readRDS(file.path( rda_path, "knn_model_1_confusionMatrix.rds"))
  knn_confusionMatrix_2 <- readRDS(file.path( rda_path, "knn_model_2_confusionMatrix.rds"))
  
  knn_confusionMatrix_1_tidy <- readRDS(file.path( rda_path, "knn_model_1_confusionMatrix_tidy.rds"))
  knn_confusionMatrix_2_tidy <- readRDS(file.path( rda_path, "knn_model_2_confusionMatrix_tidy.rds"))
  
  
  ###############################################################################
  #load randomForest objects for report
  rf_model_1_results <- readRDS(file.path( rda_path, "rf_model_1_results.rds"))
  rf_model_1_cm    <- readRDS(file.path( rda_path, "rf_model_1_confusionMatrix.rds"))
  
  rf_model_2_results <- readRDS(file.path( rda_path, "rf_model_2_results.rds"))
  
  rf_model_3 <- readRDS(file.path( rda_path, "rf_model_3.rds"))
  rf_model_3_results <- readRDS(file.path( rda_path, "rf_model_3_results.rds"))
  
  roc_predictions_1 <- readRDS(file.path( rda_path, "roc_predictions_1.rds"))
  roc_predictions_2 <- readRDS(file.path( rda_path, "roc_predictions_2.rds"))

  roc_performance_1 <- readRDS(file.path( rda_path, "roc_performance_1.rds"))
  roc_performance_2 <- readRDS(file.path( rda_path, "roc_performance_2.rds"))
  
  # load final model performace stats
  test_performance_stats <- readRDS(file.path( rda_path, "test_performance_stats_final.rds")) 
  confusionMatrix_final    <- readRDS(file.path( rda_path, "confusionMatrix_final.rds"))
  feature_importance <- readRDS(file.path( rda_path, "important_features.rds"))
...
From table above, randomForest model 3 ( _rf_model_ ) is the best performing model. Even though the performance between kNN and the last two randomForest models are very close in terms of accuracy, it it clear that randomForest model 3 is a better model using Kappa as a metric. 

However, the execution time for randomForest model 3 (rf_model_3) was over 2hours (more than the time it took for two kNN models plus the first two randomForest models). Yet, rf_model_3 only offered a small improvement over rf_model_2, Therefore, for the sake of simplicity, I selected randomForest model 2 ( __rf_model_2__ ) as the final model to apply on the validation set. In the code, this corresponds to __fit_2__ (the fitted model based on rf_model_2). The selected model used mtry of __15__.
