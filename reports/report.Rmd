...
subtitle: "Predicting Income Using Knn and randomForest"
...
```{r 01_source_include, echo=FALSE, message=FALSE, include=FALSE}
  ...
  rf_test_performance_stats <- readRDS(file.path( rda_path, "rf_test_performance_stats.rda")) 
...
```{r my_custom_themes}
    ...
    gray_chart_theme_45 <- theme_gray() + chart_main_title_theme + chart_axis_theme + angled_45_x_axis_labels_theme
...
The kNN and randomForest algorithms were chosen because they are very easy to train, and they are able handle a wide range data including a mix of categorical and numeric variables and offers good performance. 
...
The data contains 15 columns;  the _Income_ column is the response variable with the other fourteen columns being the independent variables. The structure of the dataframe looks as below:   
   
      
```{r 04_dataframe_structure, message=FALSE, warning=FALSE, echo=TRUE}
...
Based on the documentation in the _adult.names_ documentation, there are 32561 records in the training set.
...
The _hours_per_week_ variable is a little bit of bell curve with a mean and median being same ```r round(mean(adult_income$hours_per_week))```, meaning the majority of people are working fourty hours a week. However, one surprising thing is the presence of near 100hrs per week for some individuals. Although not a significant proportion, its important to note that ```r adult_income %>% filter(hours_per_week>=70) %>% nrow() ```  individuals in this dataset work over seventy hours or more per work.

The _age_ summary statistics and the histogram show a right-skewed distribution with a mean of 39 and median of 37 years. This indicates, as expected, that bulk of the workforce is concentrated in the younger age groups.  people^[https://www.bls.gov/careeroutlook/2017/article/older-workers.htm].  
...
When the categories are collapsed into more broader categories with few levels, it becomes even more clear that the vast majority of the individuals have no college degree. This may explain why the majority of the individuals earn less than 50K.  
...
``` {r education_distribution_general, fig.height=3.3, caption="Education Distribution (General Categories)" }
  adult_income %>% 
    ggplot(aes(education_collapsed, fill=education_collapsed)) +
      ...
      geom_bar() +  ggtitle("Education Distribution - New Categories")  +  
...
__Occupation__. 
Another import variable is _occupation_, a categorical variable with 15 levels and one missing (?) category.  
...
For visualization purposes, I merged the _occupation_ categories into new categories based on the the occupation classifications defined by US Bureau of Labor Statistics (BLS). These categories (classifications) are known as the Standard Occupation Classifications (SOC)
...
```{r 16_occupation_original_chart}
  ...
  categories_count %>% 
  kbl(booktabs=T, caption="Occupation by Category - Based on SOC Classifications") %>% 
    kable_styling(latex_options = c("striped", "hold_position")) %>% 
...
adult_income %>%
  ...
  ggtitle("Occupation by Category - Based on SOC Classifications") + xlab("") + 
...
```{r age_and_income_and, fig.height=3, fig.width=5}
  ...
  adult_income %>% group_by(age_group) %>% summarise(n=n(), over50K=sum(Income==">50K"), prop = round(over50K/n,3 )) %>% 
    ...
    geom_smooth(method = "loess", formula = y ~ x, se=F, size=3, color="orange" ) +
...
__Income by gender __. Gender and income inequality has always been a topic of concern in almost every society.  This section shows a simple analysis of how income differs by gender in the given dataset. The table and plots below shows that in general, a higher proportion of males earning higher income that females. Education wise, the females are equally educated as the men, but the average age for females is a bit lower than that of the males, meaning there are more younger women in the workforce (probably earning lower salaries), but am not sure if that explains the difference. 
...
The following chart and tables explore if Income is affected by capital gain or loss.
...
capital_gain_loss_proportions %>% 
  ...
  scale_fill_brewer(palette = "Set2")   +
  geom_col(width = 0.5) +  ggtitle("Proportion of Income >50K by capital gain/loss")  +  
  xlab("gain or loss") + gray_chart_theme + theme(legend.position = "none") 


adult_income %>% group_by(capital_gain_level) %>% 
  summarise(total=n(), over50K=sum(Income==">50K"), proportion=over50K/total) %>% 
...
These tasks ofr peforming the transformation are placed in R functions and can be found in the _data_engineering.R_ file. Part of it are shown below.
...
To avoid overfitting, a 5-fold cross validation was used while training the models using a sample size of 5000 from the training set.
...
The _metric_ is set to __ROC__, tells the caret to select best model using ROC rather than Accuracy.  

\newpage

### KNN Models
First, I trained a two KNN models and picked the best performinng one. My approach was, first, I trained a model using caret defaults (knn_model_1). In knn, there is only one parameter to tune k. In the dafault  model, I used a tuneLenght of 10 to allow caret to select 10 values for k. This model provided a _best_ value for k from the cross validation.  I fit that model to the entire training set, use the fit to make predictions on the test.

I take the _best k_ that value  from the first model, and expanded around it to provide more tuning values for k and used it to train another model (knn_model_2). I fitted the new model on the entire training set, and made another predictions on test. I then compare the two models. 
...
The folliwng shows the two models, the cross validation resutls, and confusion matrix.
...
We can see from the details above that in model 1 (left table), accuracy, kappa, and sensitivity are higher than those in model 2 (right table). Therefore, I selected knn_model_1 as best model from knn. Next I will train other models using randomForest and select the best model again from there. I will then  pick from a two final models, the best performing and use as my final model for validation.
  
\newpage  
  
### RandomForst Models 
#### RandomForest Model 1. Using randomForest defaults for training. 
In this section, I used the same approach as used when training the knn models. I used a sample (5K observations) to train the models. There are two main parameters of interest to me; _mtry_ (numer of random features to use in each split), and nodesize (minimum number of nodes in the terminal nodes or leaves). Number of tree (ntree) can also be set for randomForest, but in this case, I let caret choose values for ntree.  

First, I train a model using caret default ( _rf_model_1_ ), then take the best mtry from that model, expand on it and use it to train another model ( _rf_model_2_ ) to see if there is a _nodesize_ that yields a better performance. If model 2 yields an improvement, I use that k value, and the nodesize found, and train another model (rf_model_3) and see if there is an improvement. At the end, I select the best model among these three, and compare that to the one from knn. 
...
Again I set _tuneLength_ to 10 to allow caret to generate 10 random values for the tuning parameter (in this case mtry).  

The training model was then used to fit a model on the entire training set, and used it to make predictions on the test set. The results below show the results from the cross validation.  The cross validation results show that the ROC ranges from 88% to 89%. The best cross validation model achieves the 89.5% ROC at 59% sensitivity and 93% specificity.

```{r rf_model_1_cross_validation}

  rf_model_1_results %>% 
  kbl(booktabs=T, caption="Cross validation results - model 1")%>%
    kable_styling(latex_options = c("striped", "hold_position")) %>% 
    row_spec(0, bold=T) %>% 
...
With this model, the same sample (5K observations from the training set) with the same cross validation were used in an attempt to tune mtry. From model 1, it was noticed that the best mtry was ```r rf_model_1_results$mtry[which.max(rf_model_1_results$ROC)] ``` . The idea was to expand the mtry around the mtry obtained from model 1 and  see if there is a better mtry. With this approach, the best cross validation ROC for model 2 is achieved with mtry = ```r rf_model_2_results$mtry[which.max(rf_model_2_results$ROC)]```.  
...
``` {r rf_cv_results_model_2}
  rf_model_2_results %>% 
    kbl(booktabs=T, caption="Cross validation results - model 2 finding best mtry") %>%
      kable_styling(latex_options = c("striped", "hold_position")) %>% 
      row_spec(0, bold=T) %>% 
...
The following shows the cross validation results for model 2. Even though the best mtry from this model is different from the default one selected by caret in model 1, the model did not result in a significant improvement over model 1; ROC is still at 89% with both sensitivity and specificity remaining the same.

#### RandomForest Model 3. Attempt to tune randomForest nodesize parameter. 
The final two models involves attempting to tune randomforest nodesize parameter. In model 1, mtry was held constant best mtry from model 1. Different models were fitted across different values of nodesize and the one with best ROC was selected. The plot below shows the values of Kappa across different values for nodesize in model 3.

``` {r rf_model_3, fig.height=4}
...
Models 3 suggests the number ```r model_3_best_nodesize``` as an ideal value for node size. The following table shows the performance statistics for all three models fitted on the test set, including the best model from knn.
 ...
 ```{r rf_tab_final_perf_stats, caption="randomForst model performance on Test Set"}
 
 rf_test_performance_stats %>% 
    kbl(booktabs=T, caption="Model performance on Test Set") %>%
      kable_styling(latex_options = c("striped", "hold_position", "scale_down"))%>% 
      row_spec(which.max(rf_test_performance_stats$test.kappa), bold = T, color = "red") 
...
From table above, randomForest model 3 ( _rf_model_ ) is the best performing model. Even though the performance between all the models are very close, I still selected model 3 as the final model to apply on the validation because it meets my requirement of using Kappa to select the best model. The selected model used mtry of __20__ and nodesize of __45__.
...
The final confusion matrix show an accuracy of 85.8% and sentivity of 0.6. 
